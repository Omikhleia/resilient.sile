...
function package:formatMultilevelCounter (counter, options)
  ...
  local maxlevel = opts.level and SU.min(SU.cast("integer", opts.level), #counter.value) or #counter.value
  -- Option minlevel is undocumented and should perhaps be deprecated: is there a real use case for it?
  local minlevel = opts.minlevel and SU.min(SU.cast("integer", opts.minlevel, #counter.value)) or 1
  local out = {}
  if SU.boolean(opts.noleadingzeros, false) then
...
function package:registerCommands ()
  ...
  self:registerCommand("increment-counter", function (options, _)
    ...
    if (options["set-to"]) then
      SU.deprecated("\\increment-counter[set-to=...]", '\\set-counter[value=...]', "0.14.3", "0.16.0")
  ...
  self:registerCommand("show-counter", function (options, _)
    ...
    if options.display then
      SU.deprecated("\\show-counter[display=...]", '\\set-counter[display=...]', "0.14.3", "0.16.0")
  ...
  self:registerCommand("show-multilevel-counter", function (options, _)
    local counter = self.class:getMultilevelCounter(options.id)
    if options.display then
      SU.deprecated("\\show-multilevel-counter[display=...]", '\\set-multilevel-counter[display=...]', "0.14.3", "0.16.0")
