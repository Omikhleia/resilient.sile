...
local insert = function (_, classname, vbox)
  ...
  SILE.typesetter:pushMigratingMaterial({
      SILE.nodefactory.insertion({
          ...
          -- parent = SILE.typesetter.frame -- UNUSED
        })
    })
end

local function getStealFromFrames(classname)
  local opts = SILE.scratch.insertions.classes[classname]
  local insertionFrame = SILE.getFrame(opts["insertInto"].frame)
  -- If the insertion frame specifies it is definition which frames it steals
  -- space from, use these. Otherwise default to what was specified at
  -- initialization.
  if insertionFrame.stealFrom and SU.type(insertionFrame.stealFrom) ~= "table" then
    insertionFrame.stealFrom = { [insertionFrame.stealFrom] = 1 }
  end
  return insertionFrame.stealFrom or opts.stealFrom
end
...
function package:_init ()
  ...
  SILE.insertions.setShrinkage = function (classname, amount)
    local reduceList = getStealFromFrames(classname)
  ...
  SILE.insertions.commitShrinkage = function (_, classname)
    ...
    local insertionFrame = SILE.getFrame(opts["insertInto"].frame)
    local reduceList = getStealFromFrames(classname)
    local stealPosition = insertionFrame.stealPosition or opts["steal-position"] or "bottom"
  ...
  SILE.insertions.increaseInsertionFrame = function (insertionvbox, classname)
    ...
    local insertionFrame = SILE.getFrame(opts["insertInto"].frame)
    local stealPosition = insertionFrame.stealPosition or opts["steal-position"] or "bottom"
    local oldHeight = SILE.length() --+ insertionFrame:height() -- WEIRD
    print("\n-------------- INCREASE", tostring(amount), insertionFrame:height())
    amount = amount * opts["insertInto"].ratio
    if stealPosition == "bottom" then insertionFrame:relax("top") else insertionFrame:relax("bottom") end
  ...
  SILE.insertions.processInsertion = function (vboxlist, i, totalHeight, target)
    ...
    local stolenFrames = getStealFromFrames(ins.class)
    local effectOnThisFrame = stolenFrames[SILE.typesetter.frame.id]
